...
public class MainFrame extends JFrame {
    ...
    private void initComponents() {
        ...
        JPanel detailBottom = new JPanel(new FlowLayout(FlowLayout.RIGHT));
        detailBottom.add(btnDelete);
        tabDetail.add(detailBottom, BorderLayout.SOUTH);
    ...
    private void deletePost() {
        ...
        int no = (int) tableModel.getValueAt(row, 0);
        String writerId = (String) tableModel.getValueAt(row, 2);

        // 작성자 확인
        if (!loginId.equals(writerId)) {
            JOptionPane.showMessageDialog(this, "본인이 작성한 게시글만 삭제할 수 있습니다.");
            return;
        }
        ...
        DB_MAN db = new DB_MAN();
        String sql = "DELETE FROM board WHERE no = ? AND writer_id = ?";
        ...
        try {
            ...
            try (PreparedStatement ps = con.prepareStatement(sql)) {
                ...
                int ok = ps.executeUpdate();
                if (ok != 1) {
                    JOptionPane.showMessageDialog(this, "삭제 실패: 게시글을 찾을 수 없거나 권한이 없습니다.");
                    return;
                }
            }
